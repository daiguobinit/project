...
def get_complete_text_autohome(text):
    ...
    print(types)
    js_list = re.findall(r"<script>(\(function[\s\S]*)\(document\);", text)
    print(len(js_list))
    print(js_list)
    type_charlist = {}
    for js in js_list:
...
#     headers = {
#         "User-Agent": "Mozilla/6.0 (Macintosh; Intel Mac OS X 10_13_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36"}
...
def config_set():
    # 参数配置
    url = "https://car.autohome.com.cn/config/series/650.html"
    ...
    text = get_complete_text_autohome(text)
    # print(text)
    data = re.search('var config = ({"message"[\s\S]*?"time":"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}"});', text).group(1)
    print(data)
    if debug_flag:
        if "hs_kw" not in data:
            print("%s ok !!" % url)
