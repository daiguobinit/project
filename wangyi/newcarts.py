...
class WangYiSpider(object):
    ...
    def write_comment_jsonfile(self, item):
        item = json.dumps(dict(item), ensure_ascii=False) + ',\n'
        self.comments_jsonfile.write(item.encode('utf-8'))
    ...
    def run(self):
        ...
        '''
        for i in range(1, 11):  # 循环次数
            if self.is_work:
                if i == 1:
                    # ---------------------
                    url = self.nauto_url.format('')
                    print('爬取' + url)
                    self.get_all_news_page(url)
                elif i == 10:
                    # ---------------------
                    url = self.nauto_url.format('_'+str(i))
                    print('爬取' + url)
                    self.get_all_news_page(url)
                else:
                    # ---------------------
                    url = self.nauto_url.format('_0'+str(i))
                    print('爬取' + url)
                    self.get_all_news_page(url)
            else:
                break
