...
class TouTiao(object):
    ...
    def __init__(self):
        ...
        self.browser = webdriver.Chrome()  # Get local session of Chrome
        self.old_url_list_file = open('./old_url_list.json', 'r+')  # 打开文件初始化历史url列表
        self.old_url_data = self.old_url_list_file.readlines()
        self.old_url_list_file.close()  # 关闭上面的文件
        self.new_url_file = open('./new_url_file.json', 'w')  # 初始化中打开一个最新新闻的url列表的存放文件，以供爬虫的增量爬取
        self.new_url_list = []
    ...
    def get_page(self):
        self.browser.get("https://www.toutiao.com/ch/news_car/")  # Load page
        time.sleep(3)
        j = 1000  # 设定翻页的最大数， 建议不要设置太大，否者selenium会卡顿
        for i in range(j):
